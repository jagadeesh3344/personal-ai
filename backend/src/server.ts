...
export async function buildApp() {
  const app = Fastify({
    logger: process.env.LOAD_TEST === 'true' ? false : (env.NODE_ENV === 'development'),
  ...
  // 2. CORS configuration: strict in production & staging, developer friendly in dev/test
  const allowedOrigins = (env.NODE_ENV === 'production' || env.NODE_ENV === 'staging')
    ? [env.FRONTEND_URL]
  ...
  // 3. Rate Limiting (user-scoped when authenticated, IP-based fallback)
  await app.register(rateLimit, {
    max: env.NODE_ENV === 'test' || process.env.LOAD_TEST === 'true' ? 100000 : 120,
    timeWindow: '1 minute',
    keyGenerator: (req) => {
...
if (isDirectEntry && process.env.NODE_ENV !== 'test' && !process.env.VITEST) {
  startServer();
}
