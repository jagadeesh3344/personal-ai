...
export function generateWorkoutPlan(profile: UserProfile): WorkoutPlan {
  const availablePool = getAvailableExercises(profile, EXERCISE_DATABASE);
  const goal: Goal = profile.goal || 'GENERAL_FITNESS';
  const experience: TrainingExperience = profile.trainingExperience || 'BEGINNER';
  ...
  // Determine Reps and Rest based on Goal
  ...
  const helperBuildExercise = (
    exDef: typeof EXERCISE_DATABASE[0], 
    dayIndex: number, 
    orderIndex: number
  ): WorkoutExercise => {
    const isBodyweight = exDef.equipmentRequired.length === 0;
    const initialWeight = isBodyweight ? 0 : (exDef.equipmentRequired.includes('DUMBBELL') ? 12 : 30);
  ...
  const coreExercises = availablePool.filter(e => e.muscleGroups.includes('CORE'));

  // Strategy depending on workout frequency
  if (numDays <= 3) {
    // Full Body routines per day
    availableDays.forEach((dayName, idx) => {
      const dayExercises: WorkoutExercise[] = [];
      let order = 1;

      // 1 Push, 1 Pull, 1 Leg, 1 Core (if available)
      const push = pushExercises[idx % (pushExercises.length || 1)] || availablePool[0];
      const pull = pullExercises[idx % (pullExercises.length || 1)] || availablePool[1 % availablePool.length];
      const leg = legExercises[idx % (legExercises.length || 1)] || availablePool[2 % availablePool.length];
      const core = coreExercises[idx % (coreExercises.length || 1)] || availablePool[3 % availablePool.length];
  ...
  return {
    id: `plan-${Date.now()}`,
    userId: profile.id,
    name: `${profile.trainingEnvironment} ${goal.replace('_', ' ')} System`,
    goal,
