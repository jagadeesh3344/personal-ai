...
import { MEAL_RECIPES } from './recipes';
...
export function isMealSuitable(
  ...
  if (normDiet === 'VEGAN') {
    ...
    if (recipe.allergens.some(a => ['dairy', 'eggs', 'egg', 'fish', 'shellfish'].includes(a.toLowerCase()))) return false;
    ...
    if (recipe.allergens.some(a => ['fish', 'shellfish'].includes(a.toLowerCase()))) return false;
    if (recipe.ingredients.some(i => {
      const s = i.toLowerCase();
  ...
  // 2. Allergies check
  const normAllergies = (allg || []).map(a => a.toLowerCase().trim()).filter(Boolean);
  ...
  const normIntolerances = (intol || []).map(i => i.toLowerCase().trim()).filter(Boolean);
  for (const intolerance of normIntolerances) {
    ...
    if (recipe.allergens.some(a => a.toLowerCase().includes(intolerance) || intolerance.includes(a.toLowerCase()))) {
      return false;
    }
  ...
  const normDislikes = (disl || []).map(d => d.toLowerCase().trim()).filter(Boolean);
  for (const dislike of normDislikes) {
    if (recipe.ingredients.some(ing => ing.toLowerCase().includes(dislike))) {
      return false;
    }
  }
...
export function scoreMealCandidate(
  ...
  if (typeof arg2 === 'number') {
    ...
    for (const pref of preferredFoods) {
      ...
      if (candidate.name.toLowerCase().includes(p) || candidate.ingredients.some(i => i.toLowerCase().includes(p))) {
        rawScore += 25;
      }
    }
  ...
  const userPreferences = (context.foodPreferences || []).map(p => p.toLowerCase());
  for (const pref of userPreferences) {
    if (candidate.name.toLowerCase().includes(pref) || candidate.ingredients.some(i => i.toLowerCase().includes(pref))) {
      score -= 0.5;
    }
  }
...
export function generateAdaptiveMealRecommendation(
  ...
  const unloggedCount = (['BREAKFAST', 'LUNCH', 'SNACK', 'DINNER'] as MealType[])
    .filter(t => !loggedMealTypes.includes(t) || t === targetSlot).length;
  ...
  let candidates = MEAL_RECIPES.filter(r => r.mealType === targetSlot);
  ...
  candidates = candidates.filter(r => isMealSuitable(
    r,
    effectiveDiet,
    context.allergies || [],
  ...
  if (options.preferenceFilter === 'HIGH_PROTEIN') {
    candidates = candidates.filter(r => r.proteinGrams >= 30);
  } else if (options.preferenceFilter === 'LOW_CALORIE') {
    candidates = candidates.filter(r => r.calories <= 450);
  }

  if (candidates.length === 0) {
    candidates = MEAL_RECIPES.filter(r => r.mealType === targetSlot && isMealSuitable(
      r,
      context.dietPreference || 'STANDARD',
      context.allergies || [],
      context.intolerances || []
...
export function generateAdaptiveDailyMealPlan(
  ...
  const getSlotRecipe = (slot: MealType): MealRecipe => {
    ...
    const matches = MEAL_RECIPES.filter(r => r.mealType === slot);
    return matches[0];
  };
