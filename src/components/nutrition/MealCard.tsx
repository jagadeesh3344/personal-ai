...
import { Trash2, Coffee, Sun, Moon, Sparkles } from 'lucide-react';
    ...
    <Card className="p-5 bg-zinc-950/40 border-zinc-850 flex flex-col justify-between h-full" hoverEffect={true}>
      <div>
        ...
        <div className="flex items-center justify-between mb-4 pb-2 border-b border-zinc-900/40">
          ...
          <span className="text-[10px] font-mono text-zinc-500 font-semibold">{meal.totalCalories} kcal</span>
        </div>

        {/* Food list */}
        {meal.items.length === 0 ? (
          <div className="my-6 py-4 border border-dashed border-zinc-850 rounded-lg text-center bg-zinc-900/10">
            <span className="text-xs text-zinc-400 block font-medium">No foods logged</span>
            <span className="text-[9px] text-zinc-600 block mt-0.5">Record what you ate using the button below.</span>
          </div>
        ) : (
              ...
              >
                <div className="flex-1 pr-2">
                  ...
                  <div className="flex gap-1.5 items-center text-[9px] text-zinc-500 font-bold uppercase mt-0.5">
                    ...
                    <span>•</span>
                    <span className="text-rose-400/80">{item.fat || 0}g F</span>
                  </div>
                </div>
                ...
                {onDeleteFoodItem && (
                  <button 
                    ...
                    className="text-zinc-600 hover:text-red-400 p-1 rounded hover:bg-zinc-900 transition-all cursor-pointer"
                    title="Remove item"
                  >
      ...
      <div className="pt-3 border-t border-zinc-900/40 mt-3">
        <div className="flex justify-between items-center text-[10px] font-mono text-zinc-400 mb-3 px-1">
          <span>Total:</span>
          <span>{meal.totalCalories} kcal | {meal.totalProtein}g P</span>
        </div>

        <button 
          onClick={() => onAddFoodClick(meal.id)}
          className="w-full text-center text-xs font-semibold text-zinc-400 hover:text-cyan-400 border border-zinc-850 hover:border-cyan-500/30 bg-zinc-900/30 hover:bg-zinc-900/60 py-2 rounded-lg cursor-pointer transition-all uppercase tracking-wider"
        >
          + Log Food Entry
        </button>
      </div>
